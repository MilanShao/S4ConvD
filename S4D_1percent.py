...
class ASHRAEDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if input_end >= len(self.data):
            raise StopIteration("Index exceeds data limits during sequence setup.")
...
val_dataloader = DataLoader(val_dataset, batch_size=8, shuffle=False, num_workers=4, drop_last=True)
test_dataloader = DataLoader(test_dataset, batch_size=8, shuffle=False, num_workers=4, drop_last=True)
...
# S4D Cauchy kernel class and convolution
class CauchyKernelLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        dim = config["model"]["dim"]
        state_dim = config["model"]["block"]["state_dim"]

        # Parameters for Cauchy kernel
        self.log_dt = nn.Parameter(torch.rand(dim) * (np.log(0.1) - np.log(0.001)) + np.log(0.001))
        self.C_re = nn.Parameter(torch.randn(dim, state_dim // 2))
        self.C_im = nn.Parameter(torch.randn(dim, state_dim // 2))
        self.log_A_re = nn.Parameter(torch.randn(dim, state_dim // 2))
        self.A_im = nn.Parameter(torch.ones(dim, state_dim // 2))
...
class Model(nn.Module):
    def __init__(self, state_dim, measurement_dim, input_dim=4, output_dim=1):
        ...
        self.blocks = nn.ModuleList([S4DConvBlock(config) for _ in range(4)])
    ...
    def forward(self, x):
        ...
        x = self.decoder(x)
        return x[:, -1, :].squeeze()  # Squeeze to ensure shape consistency
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for batch_data in val_dataloader:
            input_sequence = batch_data["input_sequence"].to(device)
            label_sequence = batch_data["label_sequence"].to(device)
            logits = model(input_sequence).squeeze()  # Squeeze to match the shape

            # Debug statement to check tensor shapes
            print(f"Logits shape: {logits.size()}, Label sequence shape: {label_sequence.size()}")

            if logits.size() != label_sequence.size():
                print("Size mismatch detected during validation!")
                continue
            
            val_loss = rmsle(logits, label_sequence)
...
with torch.no_grad():
    for batch_data in test_dataloader:
        input_sequence = batch_data["input_sequence"].to(device)
        logits = model(input_sequence).squeeze()  # Squeeze to ensure compatibility
