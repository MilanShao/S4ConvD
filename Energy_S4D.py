...
def rmsle(predictions, targets):
    epsilon = 1e-6
    predictions, targets = predictions.squeeze(dim=-1), targets.squeeze(dim=-1)
...
class S4ConvKernel(nn.Module):
    def __init__(self, config):
        ...
        self.C_re = torch.nn.Parameter(torch.randn(H, N // 2))
        self.C_im = torch.nn.Parameter(torch.randn(H, N // 2))
        self.log_A_re = torch.nn.Parameter(torch.randn(H, N // 2))
        self.A_im = torch.nn.Parameter(torch.ones(H, N // 2))
